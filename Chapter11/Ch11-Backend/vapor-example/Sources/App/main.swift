...
drop.post("register") {
    ...
    let todoItem = User(name: userName, password: password)
    
    let todos = UserStore.sharedInstance
    todos.addOrUpdateItem(item: todoItem)
    
    let json: [User] = todos.listItems()
     return try JSON(node: resultJSON)
}

//UserStore.sharedInstance.addOrUpdateItem()
